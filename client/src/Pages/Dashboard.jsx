...
import {Card,CardBody,Tabs,Tab,TabPanel, TabsBody, TabsHeader, Typography, Button, CardFooter,Dialog,
  DialogHeader,
  DialogBody,
  ...
  const handleOpen = () => {
    ...
    useEffect(()=>{
      if(userDetail && userDetail.id){
              ...
              axios.post("http://localhost:8000/getprojects",data)
              .then(res=>{
    ...
    if(projects.length>0){
      Object.values(projects).map(project=>{console.log(project.title)})
    }
  return (
    <>
    <Nav isAuth={isAuth} />
    <div className="sec">
      <div className=" flex">
        <img src={avatar} alt="" className='h-[100px] mr-4 avatar border-solid border-black rounded' />
    ...
    <Card className=" w-96">
      <CardBody>
        ...
        <CardFooter>
        <Link to="/story">
        <Button variant='solid' className='mt-1 w-full bg-[#4d8ef7]' >
          Create
        </Button>
        </Link>
        </CardFooter>
        ...
        projects.map(project=>(
          <Card className=" w-96 mb-2" >
            <CardBody>
              ...
              <p className='text-gray text-sm italic'>Type:{project.type}</p>
              <p>{project.content.substring(0,200)}</p>
              <Button onClick={()=>{handleOpen();setSelected(project)}} variant="gradient">
        Open Dialog
      </Button>
